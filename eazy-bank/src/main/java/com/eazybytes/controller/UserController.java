...
import com.eazybytes.constants.AppliactionConstants;
...
public class UserController {
    ...
    public ResponseEntity<String> registerUser(@RequestBody Customer customer){
        try {
            ...
            if (savedCustomer.getId()>0){
                return ResponseEntity.status(HttpStatus.CREATED).body("Given user are successfully registered");
            }else {
    ...
    @RequestMapping("/user")
    public Customer getUsersDetailsAfterLogin(Authentication authentication){
    ...
    public ResponseEntity<LoginResponseDTO> apiLogin(@RequestBody LoginRequestDTO loginRequest){
        ...
        if (null != authenticationResponse&&authenticationResponse.isAuthenticated()){
            if (null != env){
                String secret = env.getProperty(AppliactionConstants.JWT_SECRET_KEY,AppliactionConstants.JWT_SECRET_DEFAULT_VALUE);
        ...
        return ResponseEntity.status(HttpStatus.OK).header(AppliactionConstants.JWT_HEADER,jwt).body(new LoginResponseDTO(HttpStatus.OK.getReasonPhrase(),jwt));
    }
}
